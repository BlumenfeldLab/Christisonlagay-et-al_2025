...
if isequal(length(plottedareas),size(combinedareas,2))
    ...
    colortable=[0 1 0; 1 .75 0; 1 .48 0; .57 .3 .98;  1 0 1; 0 .86 1; 1 0 1];
...
%     hold on
    ...
    for a=1:length(plottedareas) %1:size(table_of_faces,2) %
        ...
        side2=cell2mat(currentside);
        eval([currentarea '_' side2 '=unique(face_to_vertex1);'])
...
% 
% filename='ROIvertices.mat';
% save(filename, 'RostralInsula_lh', 'RostralInsula_rh', 'CaudalMFG_lh', 'CaudalMFG_rh', 'ComboAuditory_lh', 'ComboAuditory_rh', 'ParsOperculum_lh', 'ParsOperculum_rh', 'Supramarginal_rh', 'Supramarginal_lh', 'Thalamus_rh', 'Thalamus_lh');
