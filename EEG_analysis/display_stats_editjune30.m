% % function displayElectrodesInflated_v2_stats(data_location, save_location, patients,...
% %     overlay, createMontage, laterality, color_map, color_map_inner_bound,...
% %     color_map_outer_bounds, views, inflationstep, plot_electrodes, file_suffixes,...
% %     vertex_valuesL1, vertex_valuesR1, nparcels, varargin)
...
%                             parcel_flavor=['kmeans_nowhitemattermask'];
                            ...
                            save_location=[save_location_1 save_location_2_2 '\' save_location_2  '\' num2str(nparcels) '_parcels_' parcel_flavor  '\' laterality_label overlay '_fixed']; 
...
%       load([mappeddatadrive '\HNCT\icEEG Analysis\Analysis\EEG_behavior\normal_pipeline_file_info\normal_pipeline_file_info_20_06_09.mat']);
      load([mappeddatadrive '\HNCT\icEEG Analysis\Analysis\EEG_behavior\normal_pipeline_file_info\normal_pipeline_file_info_21_04_15.mat']);
                            ...
                            % list of all patients and suffixes
    ...
    if ~any(strcmp(overlay, overlay_labels))
        throw(MException('displayElectrodesInflated:InvalidOverlay',...
            ...
            sprintf(' ''%s''', overlay_labels{:})]))
                            ...
                            %         if laterality == 3
                            %             save_location = [save_location, '\flipX'];
                            %         end
                            ...
                            for side_index = start_side:sides_num    % Generate frames for L and R
                                ...
                                if overlay ~= 8
                                    ...
                                    for e = 1 : electrode_mapping_times
                                            ...
                                            if strcmp(overlay, 'CP')||strcmp(overlay, 'CNP')||strcmp(overlay, 'Subtraction') %overlay == 2 || overlay == 3 || overlay == 4 || overlay == 5 || overlay == 6 || overlay == 7
                                                for frame_index = 1:total_num_frames
                                                    ...
                                                    if isempty(electrode) % Some files only have electrodes on the left, hence the if statement
                                                        ...
                                                        %
                                                        [electrode_vertex_values] = electrode_data_overlay(overlay, frame_index, all_electrodes, file_suffixes{p}, params.IsVisual(p), final_bin);
                                                        
                                                        
                                                        vertex_values_tmp(:,frame_index) = electrode_vertex_values;
                                        ...
                                        ROI_electrode_values=NaN(nparcels/2,total_num_frames,elecVerticesAll_length);
                                    ...
                                    if ~ismember(overlay,1)
                                        ...
                                        sigclusters=zeros(size(inflated_surf(side_index).vertices,1),length(signal_on:total_num_frames));

                                        for frame_index = signal_on:total_num_frames %total_num_frames/2+1:total_num_frames
                                            ...
                                            if   strcmp(overlay, 'CP')||strcmp(overlay, 'CNP')||strcmp(overlay, 'Subtraction')
                                                ...
                                                if(frame_index == signal_on) % bins_for_stats) % total_num_frames/2+1)
                                                    %
                                                    chan_hood=AdjacencyMatrix;
                                                    %                     Baseline=repmat(nanmean(ROI_electrode_values(:,total_num_frames/4+1:total_num_frames/2,:),2),1,32,1);
                                                    
                                                    Baseline=repmat(nanmean(ROI_electrode_values(:,baseline_start:baseline_end,:),2),1,(total_num_frames-(signal_on-1)),1);
                                                    
                                                    [pval, t_orig, clust_info, seed_state, est_alpha]=clust_perm1_iceeg_sumt(ROI_electrode_values(:,signal_on:total_num_frames,:)-Baseline,chan_hood);
                                                ...
                                                for pcc=1:length(posn)
                                                    idpos=find(pos_clust_id(:,frame_index-(signal_on-1))==posn(pcc))
                                                    if(isempty(idpos))
                                                    else
                                                        ...
                                                        for nnn=1:length(idpos)
                                                            ...
                                                            sigclusters(find(verticesIndex==idpos(nnn)),frame_index-(signal_on-1))=1;
%                                                             if side_index==2
%                                                                 pause
%                                                             end
                                                        end
                                                ...
                                                for pcc=1:length(negn)
                                                    idneg=find(neg_clust_id(:,frame_index-(signal_on-1))==negn(pcc))
                                                    if(isempty(idneg))
                                                    else
                                                        for nnn=1:length(idneg)
                                                            vertex_values_sum(find(verticesIndex==idneg(nnn)))=-4;
                                                            sigclusters(find(verticesIndex==idneg(nnn)),frame_index-(signal_on-1))=1;
...
function [electrode_vertex_values] = electrode_data_overlay(overlay,frame_index,all_electrodes, file_suffix, IsVisual, final_bin)
overlay_frame_data = [];
...
elseif any(strcmp(overlay, {'CP', 'CNP', 'Subtraction'}))
    ...
    load(['meanpower_traces_' num2str(final_bin) 'bins_zscore_recentered_rejoutliers', file_suffix,  '.mat'])
