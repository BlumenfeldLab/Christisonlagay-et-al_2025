function displayElectrodesInflated_v2_parcellation(data_location, save_location, patients,...
...
if isnumeric(overlay)
    ...
    % Load in spectrogram times and the mni template data
    xtick_spacing_filename = 'spectrogram_times_57bins.mat';
    
    % change to local version
    mni2fs_dir = 'V:\HNCT\icEEG Analysis\Analysis\EEG_behavior\Functions for Inflated Brain Display\mni2fs-master';
    ...
    for side_index = start_side:sides_num    % Generate frames for L and R
                ...
                for p = 1 : length(patients)
                    ...
                    elseif strcmp(overlay, 'Electrode Density')
                        if isempty(electrode) % Some files only have electrodes on one hemisphere, hence the if statement
                            ...
                            [electrode_vertex_values] = electrode_data_overlay(overlay, 1, all_electrodes, file_suffixes{p}, params.IsVisual(p));
        ...
        if ~strcmp(overlay, 'Arrival Times')
            ...
            cd(image_storage_folder)
            if ~strcmp(overlay, 'Parcellation')
                audInterpPupilL;
            end
            
            if ~any(strcmp(overlay, {'Electrode Density', 'Comparison', 'Parcellation'}))
                vertex_values_sum = squeeze(nansum(vertex_values, 2));
                divisor = squeeze(sum(vertex_values ~= 0, 2));
                divisor(divisor == 0) = 1;
                
                % Take mean across patients
            ...
            elseif strcmp(overlay, 'Electrode Density')
                ...
                colormap jet
                caxis([min(vertex_values_sum(:)), 48])
              
            elseif strcmp(overlay, 'Parcellation')
                % need to add colormap as an input
                if side_index == 1
                    vertex_values_sum = vertex_valuesL1;
                else
                    vertex_values_sum = vertex_valuesR1;
                end
                colormap(color_map)
                caxis([1, nparcels])
            ...
            for frame_index = 1:total_num_frames
                ...
                if (any(strcmp(overlay, {'CP', 'CNP', 'Subtraction'})) && ndims(vertex_values) == 3) || ...
                        any(strcmp(overlay, {'Electrode Density', 'Comparison', 'Parcellation'}))% the surface is colored according to zscores of power
                    if any(electrodes_present(side_index,:) == 1) || strcmp(overlay, 'Parcellation')
                        %if ~strcmp(overlay, 'Electrode Density') && ~(strcmp(overlay, 'Comparison') && ...
                        if ~(strcmp(overlay, 'Comparison') && ...
                                strcmp(params.CompareMethod, 'XCorr')) && ~(strcmp(overlay, 'Parcellation'))
                            ...
                            alpha_data(abs(h.FaceVertexCData) <= color_map_inner_bound) = 0;
