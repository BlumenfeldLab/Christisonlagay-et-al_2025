function displayElectrodesInflated_v2(data_location, save_location, patients,...
  ...
  REQUIRED:
   ...
   - createMontage: Should be 1 if a never-before analyzed patient is
    ...
    - xtick_spacing_filename: string indicating proper time label filename.
                        these files are present in \\172.23.254.106\data25\HNCT\icEEG Analysis\Analysis\EEG_behavior\hnct_iceeg_scripting\eeg_analysis
...
SAVES:
  ...
  on the following files being present in each patient's overlay data folder:
    ...
    for side_index = start_side : sides_num
        ...
        for e = 1 : electrode_mapping_times
            ...
            for p = 1 : length(patients)
                ...
                if isempty(electrode) % Some files only have electrodes on the one side
                    ...
                    group_elec_vertices{p, side_index} = elecVertices; % saves the electrode vertices for all patients
        ...
        if ~strcmp(overlay, 'Arrival Times')
            ...
            elseif any(strcmp(overlay, {'Electrode Density', 'Subject Density'}))
                ...
                %mycmap(1 : 4 : 100, :) = [];
                %colormap(mycmap)
                load('\\MWMJ04VG66\D\HNCT\HNCT Auditory\Analysis Code\stats_and_parcellation\modifiedjet2.mat', 'mycmap')
                colormap(mycmap)
    ...
    save('group_elec_vertices.mat', 'group_elec_vertices', 'laterality')
...
elseif any(strcmp(overlay, {'CP', 'CNP', 'Subtraction'}))
    ...
    try
        ...
        disp('WARNING: Loading meanpower_traces file of time dimension total_num_frams - 1. This section of the code is intended for -2s to 2s filter 100ms bin 25ms shift data (for some sujects there are 156 bins instead of 157 bins).')
        load(['meanpower_traces_', num2str(total_num_frames - 1), 'bins_zscore_recentered_rejoutliers', file_suffix, '.mat'])
        meanpower_traces(:, :, :, 157) = zeros(size(meanpower_traces, 1), size(meanpower_traces, 2), size(meanpower_traces, 3));
    end
