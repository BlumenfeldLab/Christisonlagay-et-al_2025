function displayElectrodesInflated_v2_JD(data_location, save_location, patients,...
    ...
    xtick_spacing_filename, display_color_bar, varargin)
%{
Chris Micek Edits
...
addpath(genpath(' \\mwmj03ceuj\e\HNCT\icEEG Analysis\Analysis\EEG_behavior\hnct_iceeg_scripting\eeg_analysis'))
    ...
    for side_index = start_side : sides_num
        if side_index == 1
            ...
            if display_color_bar
                ...
                horz_dim = floor((vert_dim / size(color_bar_image, 2)) * size(color_bar_image, 1));
                for r = 1 : 3
                    color_bar_image_resize(:, :, r) = imresize(color_bar_image(:, :, 3), [vert_dim, horz_dim]);
                end
                combined_views_left{frame_index} = [combined_views_left{frame_index}, colorbar_image_resize];
            end
            ...
            if laterality == 3 && any(strcmp(overlay,{'CP', 'CNP', 'Subtraction', 'Comparison'}))
                if strcmp(overlay, 'Comparison') && strcmp(params.CompareMethod, 'XCorr')
                    ...
                    if strcmp(xtick_spacing_filename, 'spectrogram_times_57_bins.mat')
