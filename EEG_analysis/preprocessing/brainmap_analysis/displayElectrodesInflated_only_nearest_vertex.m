function displayElectrodesInflated_only_nearest_vertex(data_location, save_location,...
    patients, overlay, createMontage, laterality, views, inflationstep, electrodeDensity,...
    subtraction, file_suffixes, varargin) 
%{
Noah Freedman Edits

    Patient colors are automatically generated to maximize
    distinguishability using the function distinguishable_colors. Generated
...
Chris Micek Edits 
  ...
  contain the 'labels_all_gray_matter.mat' and
...
INPUTS:
  REQUIRED:
    ...
    - overlay: The type of overlay to use for the brain surface:
                 0 = nothing, just draw electrodes on blank brain. Saves to
                     'data_location'\Group\Electrode_Distribution
                 1 = arrival times
                 2 = zScores confirmed perceived. Saves to
                     'data_location'\Group\Gamma_power_CP
                 3 = zscores confirmed not perceived. Saves to
   ...
   - createMontage: Should be 1 if a never-before analyzed patient is
    ...
    - electrodeDensity: Boolean indicating whether to overlay electrode
                        coverage density along with electrode locations
                        (should be combined with 'overlay' = 0). You will be
                        prompted to select a 'vertex_values.mat' file
                        containing power contributions from the electrodes
                        you want to include. Saves images to
                        'data_location'\Group\Electrode_Density.

    - subtraction: Boolean indicating whether to overlay subtraction of
                   confirmed not perceived power from confirmed perceived
                   power (CP - CNP). 'overlay' should be set to 2 or 3.
                   Saves images to
...
% image_storage_folder = 'E:\Chris HNCT\icEEG Analysis\Analysis\EEG_behavior\HNCT ERP movies\Group\Inflated Surface\9 Patients Auditory ID 57 Bins CP CNP Baselines Sounds Restricted by CP Accuracy\CNP';
...
% Colors for the different patients' electrodes (old)
%{
...
for side_index = start_side:sides_num    % Generate frames for L and R
            ...
            if overlay == 2 || overlay == 3
                for frame_index = 1:total_num_frames
                    ...
                    if isempty(electrode) % Some files only have electrodes on one hemisphere, hence the if statement
                        ...
                        [vertexCdata]        = CalculateTransparency(v_num, elecVertices, electrode_vertex_values, overlay);
    ...
    if ~ismember(overlay,1)
        ...
        for frame_index = 1:total_num_frames
            ...
            if ((overlay == 2 || overlay == 3) && ndims(vertex_values) == 3) || electrodeDensity% the surface is colored according to zscores of power
                if ~electrodeDensity
                ...
                if any(electrodes_present(side_index,:) == 1)
                    if ~electrodeDensity
                        ...
                        alpha_data(abs(h.FaceVertexCData) <=  60000) = 0;
...
function [vertexCdata]= CalculateTransparency(vertices_num, elecVertices,...
    electrode_vertex_values, overlay)

vertexCdata = zeros(vertices_num, 1);
if overlay == 0
    % fill in...
else
    vertexCdata(elecVertices, :) = electrode_vertex_values;
end
end
