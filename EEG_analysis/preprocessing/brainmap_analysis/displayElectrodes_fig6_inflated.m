...
% Vertex values
load(['E:\HNCT\icEEG Analysis\Analysis\EEG_behavior\HNCT ERP movies\Group\Inflated Surface\',...
    '31 Patients Auditory ID 57 Bins Sounds Restricted by CP Accuracy 250ms\spectrogram_baseline1to0\Gamma_power_CP\vertex_values.mat'])

% Previous clustering
use_prev_cluster = 0;
cluster_file = ['E:\HNCT\icEEG Analysis\Analysis\EEG_behavior\HNCT ERP movies\Group\Inflated Surface\'...
    '31 Patients Auditory ID 77 Bins Sounds Restricted by CP Accuracy 100ms\filter_from2to2_baseline1to0r\CP_clusters\vertex_values_CP.mat'];
if use_prev_cluster
    load(cluster_file, 'vertices_by_cluster', 'vertices_by_cluster_L', 'vertices_by_cluster_R',...
        'vertices_by_cluster_reduced', 'vertices_by_cluster_reduced_200ms', 'vertices_by_cluster_reduced_400ms')
    vertices_by_cluster_prev = vertices_by_cluster;
    vertices_by_cluster_L_prev = vertices_by_cluster_L;
    vertices_by_cluster_R_prev = vertices_by_cluster_R;
    vertices_by_cluster_reduced_prev = vertices_by_cluster_reduced;
    vertices_by_cluster_reduced_200ms_prev = vertices_by_cluster_reduced_200ms;
    vertices_by_cluster_reduced_400ms_prev = vertices_by_cluster_reduced_400ms;c
end
...
image_storage_folder = ['E:\HNCT\icEEG Analysis\Analysis\EEG_behavior\HNCT ERP movies\Group\Inflated Surface\',...
    '31 Patients Auditory ID 57 Bins Sounds Restricted by CP Accuracy 250ms\spectrogram_baseline1to0\CP_clusters'];
...
poststim_reduced_200ms = poststim_reduced(:, 1 : floor(.2 / time_res));
poststim_reduced_400ms = poststim_reduced(:, 1 : floor(.4 / time_res));
...
meantrace_200ms = NaN(max_clusters, max_clusters, total_num_bins);
baseline_trace_200ms = NaN(max_clusters, stimulus_bin - 1);
poststim_trace_200ms = NaN(max_clusters, total_num_bins - stimulus_bin + 1);
meantrace_400ms = NaN(max_clusters, max_clusters, total_num_bins);
baseline_trace_400ms = NaN(max_clusters, stimulus_bin - 1);
poststim_trace_400ms = NaN(max_clusters, total_num_bins - stimulus_bin + 1);
...
for i = min_clusters : max_clusters
    ...
    if use_prev_cluster
        vertices_by_cluster_reduced_200ms(:, i) = vertices_by_cluster_reduced_200ms_prev(:, i);
    else
    ...
    for j = 1:i
        ...
        baseline_trace_200ms(j,:) = abs(meantrace_200ms(i,j,1:stimulus_bin - 1));
        poststim_trace_200ms(j,:) = abs(meantrace_200ms(i,j,stimulus_bin:total_num_bins));
    ...
    if use_prev_cluster
        vertices_by_cluster_reduced_400ms(:, i) = vertices_by_cluster_reduced_400ms_prev(:, i);
    else
        [vertices_by_cluster_reduced_400ms(:,i), ~]= kmeans(poststim_reduced_400ms, i, 'dist',...
    ...
    for j = 1 : i
        ...
        baseline_trace_400ms(j,:) = abs(meantrace_400ms(i,j,1:stimulus_bin - 1));
        poststim_trace_400ms(j,:) = abs(meantrace_400ms(i,j,stimulus_bin:total_num_bins));
