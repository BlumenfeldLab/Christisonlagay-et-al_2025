...
% addpath('D:\HNCT\HNCT Auditory\Analysis Code\stats_and_parcellation\Mass_Univariate_ERP_Toolbox-master\Mass_Univariate_ERP_Toolbox-master')
addpath('D:\HNCT\HNCT Auditory\Analysis Code\stats_and_parcellation\Mass_Univariate_ERP_Toolbox-master')
display_color_bar=1;

mappeddatadrive='V';
% % types:
% % 1. common baseline 500 ms no baseline stats
% % 2. common baseline 500 ms  baseline stats
% % 3. separate baseline 500 ms no baseline stats
% % 4. separate baseline 500 ms baseline stats
% % 5. common baseline 1000 ms no baseline stats
% % 6. common baseline 1000 ms  baseline stats
% % 7. separate baseline 1000 ms no baseline stats
% % 8. separate baseline 1000 ms baseline stats
% % 9. XFlip common baseline 500 ms no baseline stats
% % 10.XFlip  common baseline 500 ms  baseline stats
% % 11.XFlip  separate baseline 500 ms no baseline stats
% % 12.XFlip  separate baseline 500 ms baseline stats
% % 13. XFlip common baseline 1000 ms no baseline stats
% % 14.XFlip  common baseline 1000 ms  baseline stats
% % 15. XFlip separate baseline 1000 ms no baseline stats
% % 16.XFlip  separate baseline 1000 ms baseline stats

baseline_lengths=[500 1000];  %%baseline 500 or 1000 ms prestim
baseline_stats_1=[1; 39];  %%%include baseline stats or not!
flip=[0 3]; %%bilater=0; flipX=3;
baseline_status=[1,2];  %%common=1 or separate=2
for lat_plotting=1 %length(flip)
    currentstatus=flip(lat_plotting);
    laterality =currentstatus;
    
    if ~isequal(currentstatus,3)
        flipxmuliplier=1;
        laterality_label=[''];
        
    else
        flipxmuliplier=1;
        laterality_label=['flipX'];
    end
    for overlays=[3] 
        ...
        for com_or_sep=1:length(baseline_status)
            baselineID=baseline_status(com_or_sep);
            
            if isequal(baselineID,1)
                baseline_name='common';
                baselinetype='_commonr';
            else
                baseline_name='separate';
                baselinetype='r';
            end
            
            for length_baseline=1 %length(baseline_lengths)
                lengthofbaseline=baseline_lengths(length_baseline);
                
                if isequal(lengthofbaseline, 500)
                    baseline_start=19;
                    baselinesuffixes='baseline5to0';
                else
                    baseline_start=1;
                    baselinesuffixes='baseline1to0';
                    
                end
                
                
                for baseline_inclusion=1 %length(baseline_stats_1)
                    binsincludedinstats=baseline_stats_1(baseline_inclusion);
                    signal_on=binsincludedinstats;
                    
                    if isequal(signal_on,39)
                        statsinclusion=[''];
                        statsinclusionverbose=['without baseline statistics'];
                    ...
                    parceldivisions=[80];
                    for parcelsnumbers=1:length(parceldivisions)
                        nparcels=parceldivisions(parcelsnumbers);
...
%                         color_map = 'D:\HNCT\HNCT Auditory\Analysis Code\stats_and_parcellation\verysmallinnerboundscolor.mat';
                        ...
                        %                         if isequal(nparcels, 60) & isequal(baselineID,1)
%                             continue
%                         end
                            inflationstep=5;
                            %         overlay='CP';
                            ...
                            baseline_end=38;
                            final_bin=77;
                            % %         bins_for_stats=1:1:77;
%                             parcel_flavor=['kmeans_nowhitemattermask'];
                            parcel_flavor=['kmeans_mask'];
                            ...
                            save_location_3=[num2str(nparcels) '_parcels_' parcel_flavor  '\' laterality_label overlay];
%                             save_location_3=[num2str(nparcels) '_parcels_' parcel_flavor '_directional\' laterality_label overlay];
                          
                            save_location=[save_location_1 save_location_2_2 '\' save_location_2 '\'  save_location_3];
                            

                            rights=[]; lefts=[];
                            if isfile([save_location '\significantvertices_' save_location_2 '_' num2str(nparcels) 'parcels.mat'])
                                ...
                                load([save_location '\significantvertices_' save_location_2 '_' num2str(nparcels) 'parcels.mat'])
                            else
                                ...
                                if ~isequal(currentstatus,3)
                                    for a=1:100
                                        ...
                                        load ([save_location '\significant_vertices_map' num2str(a) '.mat'])
                                    ...
                                    save([save_location '\significantvertices_' save_location_2 '_' num2str(nparcels) 'parcels.mat'], 'sign_left', 'sign_right');
                                    ...
                                    save([save_location '\significantvertices_' save_location_2 '_' num2str(nparcels) 'parcels.mat'], 'sign_left');
                            ...
                            significant_vertices_filename=[save_location '\significant_vertices_zscores.mat'];
                            disp(['Running stats for ' laterality_label ' ' overlay ' using with ' num2str(nparcels) ' parcels, with a ' num2str(lengthofbaseline) 'ms ' baseline_name ' baseline '  statsinclusionverbose])
                            ...
                            if strcmp(baselinetype, '_commonr')
                                file_suffixes=eval(['mean_suffixes_77bins_100ms_filter_from2to2_' baselinesuffixes baselinetype]);
                            else
                            ...
                            %         xtick_spacing_filename = ['N:\HNCT\icEEG Analysis\Analysis\EEG_behavior\hnct_iceeg_scripting\eeg_analysis\spectrogram_times_' num2str(final_bin) 'bins.mat'];
  ...
  on the following files being present in each patient's overlay data folder:
    ...
    - 'R_MontageMap.mat': Contains the data from 'MontageMap.mat' belonging
                          to the right hemisphere.
                            ...
                            daterun = date;
                save([save_location '\vertexsum_' save_location_2 '_' num2str(nparcels) 'parcels.mat'], 'daterun' )            
                            ...
                            new_color=['_colorbar040121'];
                            
                            switch overlay
                                case 'Electrode Distribution'
                                    image_storage_folder = [save_location '/Electrode_Distribution'];
                                case 'CP'
                                    image_storage_folder = [save_location '/Gamma_power_CP' new_color];
                                case 'CNP'
                                    image_storage_folder = [save_location '/Gamma_power_CNP' new_color];
                                case 'Electrode Density'
                                    image_storage_folder = [save_location '/Electrode_Density'];
                                case 'Subtraction'
                                    image_storage_folder = [save_location '/Gamma_power_Subtraction' new_color];
                                case 'Comparison'
                ...
                caxis([min([vertex_values_sum_L; vertex_values_sum_R]),...
                    max([vertex_values_sum_L; vertex_values_sum_R])])
                                    ...
                                    if isequal(side_index,1)
                                        ...
                                        save([save_location '\vertexsum_' save_location_2 '_' num2str(nparcels) 'parcels.mat'], 'leftverts', '-append');
                                    elseif isequal(side_index,2)
                                        rightverts=vertex_values_sum;
                                        save([save_location '\vertexsum_' save_location_2 '_' num2str(nparcels) 'parcels.mat'], 'rightverts', '-append');
                                    end
                                        ...
                                        if (any(strcmp(overlay, {'CP', 'CNP', 'Subtraction'})) && ndims(vertex_values) == 3) || ...
                                            ...
                                            if any(electrodes_present(side_index, :) == 1)
                                                ...
                                                if ~(strcmp(overlay, 'Comparison') && strcmp(params.CompareMethod, 'XCorr'))
                                                    ...
                                                    h = patch('Faces',inflated_surf(side_index).faces, 'Vertices', inflated_surf(side_index).vertices,...
                                                        'FaceVertexCData',(vertex_values_sum(:, frame_index).*sign_vertices(:,frame_index)),'FaceColor','interp',...
                                                        'LineStyle', 'none', 'FaceAlpha', 'flat');
                                                    
                                                    
                                                    if ~strcmp(overlay, 'Electrode Density')
                                                        bloop=1
...
%%set opacity
                                                    ...
                                                    if ~isequal(max(sign_vertices(:,frame_index)),0)
...
%                                 save([save_location '\vertexvalues_' save_location_2 '_' num2str(nparcels) 'parcels.mat'],'vertex_valuesL','vertex_valuesR','-v7.3')

                            elseif laterality == 3 || laterality == 1
                                save('vertex_values.mat','vertex_valuesL','-v7.3')
%                                 save([save_location '\vertexvalues_' save_location_2 '_' num2str(nparcels) 'parcels.mat'],'vertex_valuesL','-v7.3')

                            elseif laterality == 2
                                save('vertex_values.mat','vertex_valuesR','-v7.3')
%                                 save([save_location '\vertexvalues_' save_location_2 '_' num2str(nparcels) 'parcels.mat'],'vertex_valuesR','-v7.3')
                                        ...
                                        if laterality == 3 && any(strcmp(overlay,{'CP', 'CNP', 'Subtraction', 'Comparison'}))
                                            if strcmp(overlay, 'Comparison') && strcmp(params.CompareMethod, 'XCorr')
                                                time_txt = ['Lag = ' num2str(round((T2(frame_index))*1000)) 'ms'];
                                                                                            ...
                                                                                            t_num = round((T(frame_index) - 1) * 1000);
                                            ...
                                            elseif strcmp(xtick_spacing_filename_2, 'filter_times_win100ms_shift50ms.mat') ||strcmp(xtick_spacing_filename_2, 'filter_times_win100ms_shift25ms.mat')
                                            ...
                                            text(size(combined_views_left{frame_index}, 2) - uc.Extent(3) - 20,...
                                                size(combined_views_left{frame_index}, 1) - uc.Extent(4) - 20, time_txt, 'Fontsize', 32)
                                        end
                                ...
                                if laterality == 0
                                    ...
                                    if any(strcmp(overlay, {'CP', 'CNP', 'Subtraction', 'Comparison'}))
                                        ...
                                        if views == 4
                                            ...
                                            text(size(combined_views, 2) - uc.Extent(3) - 20,...
                                                size(combined_views, 1) - uc.Extent(4) - 20, time_txt, 'Fontsize', 32)
                                        end
