n%%%written by Kate Christison-Lagay in April 2021; modified from previous
...
%     load HOArefinedtable_min200_smoothing6_lh_13-May-2021_version11.mat
...
%     load HOArefinedtable_min200_smoothing6_rh_13-May-2021_version16.mat
...
%             min(find(proportion_sig_1>0))
            ...
            avactivity=avactivity(:, firstbin:finalbin);
            if ~isequal(sum(~isnan(avactivity)),0)
                ...
                stderrorofmean=stderrorofmean(1, firstbin:finalbin);
                options.error='sem';
...
% lowerbound=-35;
% upperbound=20;
                activity_insignverts_inregion=activity_insignverts_inregion(:,firstbin:finalbin);
                ...
                xticks([[firstbin neg250bin stimon pos250bin pos500bin pos750bin]-firstbin+1]);
                xticklabels({'-500', '-250', '0', '250', '500', '750'});
                                        xlabel('ms')
                %                         axis([19 77.5 -3 33])
                %                         axis([19 77.5 -10 18])
                
                axis([firstbin-firstbin+1 (finalbin)-firstbin+1.5 lowerbound upperbound])
                xline(stimon+(-firstbin+1)+3, '--')
                xline(stimon+(-firstbin+1), '--')
                ...
                proportion_sig=proportion_sig(:, firstbin:finalbin)
                sigarea=area(((proportion_sig*multiplier)+lowerbound), lowerbound, 'FaceColor', colorline);
%                 sigbars=bar((proportion_sig*multiplier)+lowerbound, widthfactor, 'FaceColor', colorline);
                
                sigarea.FaceAlpha=.5;
                breaches50=find(proportion_sig>.5);
                reindexed_ind=min(find(breaches50>(stimon-firstbin+1)));
                first50plus=breaches50(reindexed_ind);
                if ~isempty(first50plus)
                    scatter(first50plus, (signstar*multiplier)+lowerbound, '*', 'MarkerEdgeColor', colorline)
                end
                
                axis([firstbin+(-firstbin+1) (finalbin)+(-firstbin+1)+.5 lowerbound upperbound])
%                 sigbars.BaseValue=lowerbound;
...
%                 xticklabels({'-500', '-250', '0', '250', '500', '750'});
%                 xlabel('ms')
                yticks([lowerbound lowerbound+multiplier]);
                yticklabels([0 1])
                counter=counter+1;
