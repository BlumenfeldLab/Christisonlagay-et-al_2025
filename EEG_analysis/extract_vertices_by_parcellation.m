...
for overlays=[1:3]
    if isequal(overlays, 1)
        ...
        %%start_pt is the first map used
        overlay='CP';
        start_pt=44; 
    ...
    for maps=start_pt:100
        ...
        elseif any(strcmp(overlay, {'CP', 'CNP', 'Subtraction', 'Comparison'}))
            ...
            if strcmp(overlay, 'Comparison') && strcmp(params.CompareMethod, 'XCorr')
                ...
                total_num_frames = numel(T)-1; %%the minus 1 here is b/c some pts have 156 bins b/c of sampling rate diffs
            end
        end
        ...
        for side_index = start_side:sides_num    % Generate frames for L and R
            ...
            if overlay ~= 8
                ...
                for e = 1 : electrode_mapping_times
                    ...
                    for p = 1 : length(patients)
                        ...
                        if strcmp(overlay, 'CP')||strcmp(overlay, 'CNP')||strcmp(overlay, 'Subtraction') %overlay == 2 || overlay == 3 || overlay == 4 || overlay == 5 || overlay == 6 || overlay == 7
                            for frame_index = 1:total_num_frames  %%only want from baseline to the end of desired period
                ...
                if side_index == 1
                    vertex_valuesL = ROI_electrode_values;
                    verticesIndexL= verticesIndex;
                    chan_hoodL=AdjacencyMatrix;
                    
                    if exist(significant_vertices_filename, 'file')
                        save(significant_vertices_filename,'vertex_valuesL', 'verticesIndexL', 'chan_hoodL', '-append')
                    else
                        save(significant_vertices_filename,'vertex_valuesL', 'verticesIndexL', 'chan_hoodL')
                    end
                    
                elseif side_index == 2
                    vertex_valuesR = ROI_electrode_values;
                    verticesIndexR= verticesIndex;
                    chan_hoodR=AdjacencyMatrix;

                    
                    if exist(significant_vertices_filename, 'file')
                        save(significant_vertices_filename,'vertex_valuesR', 'verticesIndexR', 'chan_hoodR', '-append')
                    else
                        save(significant_vertices_filename,'vertex_valuesR', 'verticesIndexR', 'chan_hoodR')
                    end
                end
